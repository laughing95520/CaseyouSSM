	...
	<body>
		...
		<div id="content">
			<div id="public_right">
				...
				<p id="check" style="display: none;">${username}</p>
				...
				<div class="article-content">
					...
					<ul class="article-next-prev">
						...
						<span>上一篇</span>
						<c:choose>
							<c:when test="${not empty article.up_id}">
								<c:if test="${action=='like' }">
								<a href="article_page_By_like.action?id=${article.up_id}">${article.up_name}</a>
								</c:if>
								<c:if test="${action=='download' }">
									<a href="article_page_By_download.action?id=${article.up_id}">${article.up_name}</a>
								</c:if>
								<c:if test="${action=='view' }">
								<a href="article_page_By_view.action?id=${article.up_id}">${article.up_name}</a>
								</c:if>
								<c:if test="${action=='time' }">
								<a href="article_page_By_time.action?id=${article.up_id}">${article.up_name}</a>
								</c:if>
								<c:if test="${action=='search' }">
								<a href="article_page_By_search.action?id=${article.up_id}">${article.up_name}</a>
								</c:if>
							</c:when>
							<c:otherwise>
								<span>亲，前面没有了=_=</span>
						...
						<c:choose>
							<c:when test="${not empty article.next_id}">
								<c:if test="${action=='like' }">
								<a href="article_page_By_like.action?id=${article.next_id}">${article.next_name}</a>
								</c:if>
								<c:if test="${action=='download' }">
									<a href="article_page_By_download.action?id=${article.next_id}">${article.next_name}</a>
								</c:if>
								<c:if test="${action=='view' }">
								<a href="article_page_By_view.action?id=${article.next_id}">${article.next_name}</a>
								</c:if>
								<c:if test="${action=='time' }">
								<a href="article_page_By_time.action?id=${article.next_id}">${article.next_name}</a>
								</c:if>
								<c:if test="${action=='search' }">
								<a href="article_page_By_search.action?id=${article.next_id}">${article.next_name}</a>
								</c:if>
							</c:when>
							<c:otherwise>
